...
def run(rank, n_gpus, hps):
    ...
    if rank == 0:
        ...
        logger.info(hps)
        util.check_git_hash(hps.model_dir)
        ...
        try:
            if net_dur_disc is not None:
                _, optim_dur_disc, _, epoch_str = util.load_checkpoint(
                    util.latest_checkpoint_path(hps.model_dir, "DUR_*.pth"), net_dur_disc, optim_dur_disc,
                    skip_optimizer=not hps.cont)
            _, optim_g, _, epoch_str = util.load_checkpoint(util.latest_checkpoint_path(hps.model_dir, "G_*.pth"),
                                                             net_g,
                                                             optim_g, skip_optimizer=not hps.cont)
            _, optim_d, _, epoch_str = util.load_checkpoint(util.latest_checkpoint_path(hps.model_dir, "D_*.pth"),
                                                             net_d,
    ...
    for batch_idx, (x, x_lengths, spec, spec_lengths, y, y_lengths, speakers, tone, language, bert) in tqdm(
        ...
        if rank == 0:
            ...
            if global_step % hps.train.eval_interval == 0:
                ...
                util.save_checkpoint(net_g, optim_g, hps.train.learning_rate, epoch,
                                      os.path.join(hps.model_dir, "G_{}.pth".format(global_step)))
                util.save_checkpoint(net_d, optim_d, hps.train.learning_rate, epoch,
                                      os.path.join(hps.model_dir, "D_{}.pth".format(global_step)))
                if net_dur_disc is not None:
                    util.save_checkpoint(net_dur_disc, optim_dur_disc, hps.train.learning_rate, epoch,
                                          os.path.join(hps.model_dir, "DUR_{}.pth".format(global_step)))
                keep_ckpts = getattr(hps.train, 'keep_ckpts', 5)
                if keep_ckpts > 0:
                    util.clean_checkpoints(path_to_models=hps.model_dir, n_ckpts_to_keep=keep_ckpts, sort_by_time=True)
